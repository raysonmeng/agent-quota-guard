#!/usr/bin/env node
// budget-guard — Claude Code / Codex hook core (Node >=18, zero deps).
//
...
//   pre     PreToolUse         — hard line / runway: slowdown reminder only
...
// `触发窗口` is the warn_util winner (usage.warn_bucket_id) — the bucket actually
// at the top across ALL util-bearing windows (including non-resettable ones).
// The hard line gates on warn_util, so naming the warn winner keeps the labeled
// window consistent with the headline percentage. It equals bucket_id (the
// resettable winner) in the common case where the top window is resettable, and
// only diverges when a non-resettable bucket is the real max. Falls back to
// bucket_id for older probes that predate warn_bucket_id. The full `窗口:` list
// always shows every bucket's real util regardless.
function usageDetail(usage) {
  const warnId = typeof usage?.warn_bucket_id === 'string' ? usage.warn_bucket_id.trim() : '';
  const bucketId = typeof usage?.bucket_id === 'string' ? usage.bucket_id.trim() : '';
  const triggerId = warnId || bucketId;
  const buckets = Array.isArray(usage?.buckets)
    ...
    : [];
  const parts = [];
  if (triggerId) parts.push(`触发窗口:${triggerId.replace(/\s+/g, '_')}`);
...
function activeRateLimitUntil(usage) {
  const until = Number(usage?.rate_limited_until);
  return Number.isFinite(until) && until > nowSec() ? until : 0;
}

function checkpointLead(th) {
  const lead = Number(th?.checkpointLead);
  return Number.isFinite(lead) ? lead : Number(th?.hard);
}

function hasNumericUtil(usage) {
  return !!usage && (Number.isFinite(usage.warn_util) || Number.isFinite(usage.util));
}

/**
 * Whether usage can drive threshold decisions this poll.
 *
 * A transient probe HTTP 429 (rate_limited_until in the future) means the LIVE
 * probe could not refresh — it is NOT subscription-quota exhaustion. Quota is a
 * utilization percentage and never surfaces as a 429; a 429 is the metering
 * endpoint rate-limiting bursty requests (e.g. a fan-out of parallel agents).
 * So the gate must NEVER write a handoff pending or force a stop on its own.
 * Instead we evaluate the (stale but numeric) cached util exactly like fresh
 * data: usable when the probe is ok, OR a gate is active AND the cache still
 * carries a numeric util to judge. A genuine probe failure with no cached util
 * stays fail-open (silent).
 */
function usageEvaluable(usage) {
  if (!usage) return false;
  if (usage.ok) return true;
  // Gated path: ONLY a genuine stale cache counts (probe served an earlier live
  // reading). The no-cache gated path returns an emptyResult placeholder with
  // util/warn_util = 0 (stale:false, source:'live') — that is NOT data and must
  // stay fail-open silent, or we would fabricate a "0% used, all healthy"
  // reading. Discriminate on stale/source, not just numeric util (Codex catch).
  return (
    activeRateLimitUntil(usage) > 0 &&
    (usage.stale === true || usage.source === 'cache') &&
    hasNumericUtil(usage)
  );
}

/**
 * Non-imperative suffix noting the probe is serving stale data under a transient
 * 429. Informational ONLY — it never tells the agent to stop / checkpoint /
 * hand off. Empty string when no gate is active. Appended to a message the util
 * thresholds were ALREADY going to emit; a healthy util emits nothing at all.
 */
function staleProbeNote(usage) {
  const until = activeRateLimitUntil(usage);
  if (until <= 0) return '';
  const retry = fmtClock(until);
  const u = Number.isFinite(usage.warn_util) ? usage.warn_util : usage.util;
  const utilPart = Number.isFinite(u) ? `缓存额度 util ${u}%` : '缓存额度';
  const retryPart = retry !== '未知' ? `，约 ${retry} 恢复刷新` : '';
  return `（探针暂被限流，以下基于${utilPart}${retryPart}）`;
}
...
export function finishingHorizonSec() {
  const raw = process.env.BUDGET_FINISHING_HORIZON;
  if (raw !== undefined && /^\d+$/.test(String(raw).trim())) {
    return Number.parseInt(String(raw).trim(), 10);
  }
  return 1800; // 30 min
...
function burnToFull(bucket) {
  const cappedRunway = Number(bucket?.runway_seconds);
  if (!Number.isFinite(cappedRunway)) return null;
  const util = Number(bucket?.util);
  const rate = Number(bucket?.burn_rate_pct_per_hour);
  if (!Number.isFinite(util) || !Number.isFinite(rate) || rate <= 0) return null;
  const burnSeconds = Math.max(0, Math.round(((100 - util) / rate) * 3600));
  return { burnSeconds, resetBound: cappedRunway < burnSeconds };
}

export function selectFinishingBucket(usage, horizonSec) {
  const warnId = typeof usage?.warn_bucket_id === 'string' ? usage.warn_bucket_id.trim() : '';
  const bucketId = typeof usage?.bucket_id === 'string' ? usage.bucket_id.trim() : '';
  const winnerId = warnId || bucketId;
  const buckets = Array.isArray(usage?.buckets) ? usage.buckets : [];
  const winnerBucket = buckets.find((b) => b && b.id === winnerId) || null;

  // Finishing is a fleet-wide signal, not a warn-winner property: the warn winner
  // is purely the highest-util bucket (probe warnWinner ignores burn rate), so a
  // low-util bucket that is filling fast would be missed if we only inspected the
  // winner. Walk EVERY bucket and pick the one with the smallest GENUINE
  // (uncapped) burn-to-full. burnToFull keeps the genuine burnSeconds and flags
  // reset-bound buckets (runway_seconds truncated below the true burn-to-full):
  // those can never genuinely fill before their reset, so they are excluded here
  // — this preserves the reset-bound fix (reset-bound buckets stay static, never
  // strong, and never get a "距打满" line).
  let fastest = null; // { bucket, burnSeconds }
  for (const b of buckets) {
    if (!b || b.burn_confident !== true) continue;
    const burn = burnToFull(b);
    if (burn === null || burn.resetBound === true) continue;
    if (fastest === null || burn.burnSeconds < fastest.burnSeconds) {
      fastest = { bucket: b, burnSeconds: burn.burnSeconds };
    }
  }

  if (fastest !== null) {
    return {
      level: fastest.burnSeconds < horizonSec ? 'strong' : 'soft',
      bucket: fastest.bucket,
      runwaySeconds: fastest.burnSeconds,
      horizonSec,
    };
  }
  return { level: 'static', bucket: winnerBucket, runwaySeconds: null, horizonSec };
}

// Render the "按当前消耗速度…距打满 X" clause for strong/soft finishing levels.
//
// CRITICAL cross-bucket consistency: the runway/burn-to-full figure belongs to
// finishing.bucket (the fastest-FILLING bucket selectFinishingBucket walked the
// whole fleet to find), which is NOT necessarily the headline's warn winner
// (highest-UTIL bucket — what `base` / usageDetail / 触发窗口 describe). When the
// two diverge, an un-qualified "距打满 15 分钟" reads as belonging to the headline
// percentage's window, which is wrong (that window may fill in hours). So we name
// finishing.bucket — its id + its own util — right on the burn-to-full clause, so
// the duration is unambiguously attributed to the bucket it was computed from.
// The headline's hard line / 触发窗口 stay as-is (they correctly describe the warn
// state). reset-bound / static levels never reach here (no "距打满").
function finishingClause(finishing) {
  const bucket = finishing?.bucket;
  const id = typeof bucket?.id === 'string' ? bucket.id.trim() : '';
  const util = Number(bucket?.util);
  const label = id ? id.replace(/\s+/g, '_') : '';
  // Name the bucket the runway was computed from: "窗口 five_hour(70%)…".
  if (label && Number.isFinite(util)) {
    return `窗口 ${label}(${Math.floor(util)}%)按当前消耗速度预计距打满 ${fmtDuration(finishing.runwaySeconds)}`;
  }
  if (label) {
    return `窗口 ${label} 按当前消耗速度预计距打满 ${fmtDuration(finishing.runwaySeconds)}`;
  }
  return `按当前消耗速度预计距打满 ${fmtDuration(finishing.runwaySeconds)}`;
}

function phasePreMessage(usage, hardUtil, th, cp, finishing) {
  const reset = fmtClock(usage.reset_epoch);
  const resetPart = reset !== '未知' ? ` 刷新约 ${reset}。` : '';
  const detail = usageDetail(usage);
  const lead = checkpointLead(th);
  const base = hardUtil >= th.hard
    ? `额度 ${hardUtil}%≥${th.hard}% 硬线${detail}。`
    : hardUtil >= lead
      ? `额度 ${hardUtil}%≥checkpoint 提醒线 ${lead}%(硬线 ${th.hard}%)${detail}。`
    : `额度 ${hardUtil}%(硬线 ${th.hard}%)${detail}。`;
  const noDeny = '不会强制拦截新工具，但接近供应商限流时可能被外层硬切。';

  if (finishing?.level === 'strong') {
    return `${base}${finishingClause(finishing)}，`
      + `低于收尾窗口 ${fmtDuration(finishing.horizonSec)}。立即把进度写进 ${cp} 并收尾到干净点；${noDeny}${resetPart}`;
  }

  if (finishing?.level === 'soft') {
    return `${base}${finishingClause(finishing)}，`
      + `高于收尾窗口 ${fmtDuration(finishing.horizonSec)}。继续推进但保持 checkpoint 节奏，必要时写 ${cp}；${noDeny}${resetPart}`;
  }

  return `${base}请尽快把进度写进 ${cp} 并收尾到干净点；${noDeny}${resetPart}`;
}
...
export async function phasePrompt(agent, input, th) {
  ...
  // A transient probe 429 is NOT quota exhaustion: never stop / writePending on
  // the gate alone. Evaluate the (stale) cached util like fresh data instead.
  if (!usageEvaluable(usage)) return null;
...
export async function phasePre(agent, input, th) {
  ...
  // tools without a warning when a non-resettable bucket is the one maxed out
  // (warn_util high, hard_max 0) right up until the Stop boundary. All three
  // phases now agree on warn_util.
  const hardUtil = Number.isFinite(usage.warn_util) ? usage.warn_util : usage.util;
  const finishing = selectFinishingBucket(usage, finishingHorizonSec());
  const lead = checkpointLead(th);
  const isHardLine = hardUtil >= th.hard;
  const isCheckpointLead = hardUtil >= lead;
  if (!isHardLine && !isCheckpointLead && finishing.level !== 'strong') return null;

  // Slowdown line. Checkpoint writes are already the desired recovery artifact.
  const cp = checkpointPath();
  ...
  const message = phasePreMessage(usage, hardUtil, th, cp, finishing) + staleProbeNote(usage);
  ...
  if (agent === 'codex') return { systemMessage: message };
  return null;
}
...
export async function phasePost(agent, input, th) {
  ...
  } else if (warnUtil >= lead) {
    message = `额度已用 ${warnUtil}%≥checkpoint 提醒线 ${lead}%(硬线 ${th.hard}%)${usageDetail(usage)}。`
      + `现在开始保持收尾节奏,把进度写进 ${cp};硬线只是最后保险丝,别等 ${th.hard}% 才 checkpoint。`;
...
export async function phaseStop(agent, input, th) {
  const usage = await fetchUsage(agent);
  // A transient probe 429 is NOT quota exhaustion and MUST NOT force a stop. The
  // only legitimate hard stop is the genuine util-hard-line below, evaluated on
  // the (stale) cached util. Never `continue:false` / writePending on the gate
  // alone — that was the false-stop-at-healthy-quota bug.
  if (!usageEvaluable(usage)) return null;
  ...
  if (warnUtil >= th.hard) {
    ...
    // When the hard line was read off a stale cache (probe gated by a transient
    // 429), say so — the stop is genuine (util≥hard last we knew) but the caveat
    // keeps the message honest and consistent with phasePre/phasePost.
    stopReason += staleProbeNote(usage);
  ...
  if (warnUtil >= lead) {
    if (agent === 'codex') {
      return { systemMessage: `额度 ${warnUtil}%≥checkpoint 提醒线 ${lead}%(硬线 ${th.hard}%),建议本轮收尾并写 ${cp}。${staleProbeNote(usage)}` };
    }
    return null;
  }
  ...
  if (warnUtil >= th.warnRepeat) {
    ...
    if (agent === 'codex') {
      return { systemMessage: `额度 ${warnUtil}%,建议本轮收尾并写 ${cp}。${staleProbeNote(usage)}` };
    }
    return null;
  }
...
// Read the cwds recorded in this agent's pending files, newest-first by mtime,
// de-duplicated. Returns [] when none exist. The caller picks the first cwd that
// belongs to the current project, so several concurrent projects' pending files
// don't make a more-recently-stopped UNRELATED project win the fallback.
//
// "Newest-first" is by file mtime (NOT readdir order, which is filesystem /
// alphabetical). The legacy flat file is the last candidate.
function pendingCwds(agent) {
  const dir = stateDir();
  let scoped = [];
  try {
    const pendingDir = join(dir, 'pending');
  ...
  for (const p of candidates) {
    try {
      ...
      if (typeof obj.cwd === 'string' && obj.cwd.length > 0 && !cwds.includes(obj.cwd)) {
        cwds.push(obj.cwd);
      }
...
// Resolve a path's git "common dir" (absolute, normalized). All worktrees of a
// repository share the SAME common dir, so this groups a repo and its worktrees
// together while keeping unrelated projects distinct. Returns null when the path
// is not inside a git repo or git is unavailable (best-effort, short timeout).
function gitCommonDir(dir) {
  try {
    ...
    if (!out) return null;
    const abs = isAbsolute(out) ? out : resolvePath(dir, out);
    // Canonicalize symlinks (macOS /var → /private/var etc.) so the same repo
    // compares equal whether git reported it from the root or a subdir.
    try { return realpathSync(abs); } catch (_) { return resolvePath(abs); }
  } catch (_) {
    return null;
  }
}

// True when two cwds belong to the same project: identical realpaths, or the
// same git repository (including its worktrees). Used to scope the resume
// fallback so opening an UNRELATED project never resumes another project's
// checkpoint. Two non-git dirs only match when their realpaths are identical.
function sameProject(a, b) {
  // realpathSync canonicalizes symlinks (matching gitCommonDir's normalization);
  // resolvePath is the non-throwing fallback when a path doesn't exist on disk.
  const canon = (p) => { try { return realpathSync(p); } catch (_) { return resolvePath(p); } };
  if (canon(a) === canon(b)) return true;
  const ca = gitCommonDir(a);
  const cb = gitCommonDir(b);
...
export async function phaseResume(agent, input, th) {
  ...
  if (!body && !overrideCwd && !isAbsolute(cp)) {
    // cwd may have changed between sessions (e.g. a worktree); try the cwds from
    // recent pending files — but ONLY one that belongs to the SAME project as
    // the current cwd. Otherwise opening an unrelated project would resume a
    // stale checkpoint from whatever project hard-stopped last.
    for (const savedCwd of pendingCwds(agent)) {
      if (!sameProject(resolvedBase, savedCwd)) continue;
      body = readBody(resolvePath(savedCwd, cp));
      if (body) break;
    }
  }
  if (!body) return null;
...
export async function runDoctor(agent, th) {
  ...
  process.stdout.write(`  thresholds: warnOnce=${th.warnOnce} warnRepeat=${th.warnRepeat} checkpointLead=${checkpointLead(th)} hard=${th.hard}\n`);
