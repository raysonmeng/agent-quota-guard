...
function rateLimitMessage(usage, cp) {
  const until = activeRateLimitUntil(usage);
  const retry = fmtClock(until);
  const retryPart = retry !== '未知' ? `重试窗口约 ${retry}。` : '';
  return `供应商已返回限流(rate-limit/429),守卫已写入待续 pending。`
    + `请确认当前进度已写进 ${cp};${retryPart}额度恢复后由 bridge/watchdog 从 checkpoint 续接。`;
}
...
export async function phasePrompt(agent, input, th) {
  ...
  if (activeRateLimitUntil(usage) > 0) {
    writePending(agent, input, usage);
    const message = rateLimitMessage(usage, checkpointPath());
    if (agent === 'claude') {
      return {
        hookSpecificOutput: {
...
export async function phasePre(agent, input, th) {
  ...
  const message = phasePreMessage(usage, hardUtil, th, cp, finishing);
...
export async function phaseStop(agent, input, th) {
  ...
  if (activeRateLimitUntil(usage) > 0) {
    writePending(agent, input, usage);
    const retry = fmtClock(activeRateLimitUntil(usage));
    let stopReason = `供应商已返回限流(rate-limit/429),守卫已写入待续 pending,本轮干净停下。`;
    if (retry !== '未知') stopReason += `重试窗口约 ${retry}。`;
    stopReason += `额度恢复后请调 wait_until_budget_refresh 或由 watchdog/bridge 从 checkpoint 续跑。`;
    if (agent === 'codex') {
  ...
  if (warnUtil >= lead) {
    if (agent === 'codex') {
      return { systemMessage: `额度 ${warnUtil}%≥checkpoint 提醒线 ${lead}%(硬线 ${th.hard}%),建议本轮收尾并写 ${cp}。` };
    }
    return null;
  }
