...
export async function phasePre(agent, input, th) {
  ...
  const reason = `额度 ${hardUtil}%≥${th.hard}% 硬线。停止新工作,只写 ${cp},别绕。`
    + (reset !== '未知' ? ` 刷新约 ${reset}。` : '');
...
export async function phasePost(agent, input, th) {
  ...
  if (warnUtil >= th.hard) {
    ...
    // per hard crossing (fingerprint keyed on th.hard).
    if (shouldFire(agent, warnUtil, bucketId, resetEpoch, th.hard)) {
      message = `额度已用 ${warnUtil}%≥硬线 ${th.hard}%。本轮 Stop 钩子将强停:`
...
export async function phaseStop(agent, input, th) {
  ...
  if (warnUtil >= th.hard) {
    ...
    const reset = fmtClock(usage.reset_epoch);
    let stopReason = `额度 ${warnUtil}%≥${th.hard}% 硬线,已干净停下。`
      + (reset !== '未知' ? `刷新约 ${reset}。` : '')
