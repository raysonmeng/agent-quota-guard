...
//
// Pure ESM, zero external deps, Node 22+. Self-deploys the installer payload
 ...
 */
function deployTree(srcDir, destDir, { required, optional = [], kindLabel = srcDir } = {}) {
  ...
  for (const name of files) {
    ...
    if (basename(dst) === 'probe.mjs' || basename(dst) === 'guard.mjs') {
...
function step1_deploy({ srcRoot, binDir, libDir, label = '部署脚本' }) {
  ...
  const probeBin = deployTree(join(srcRoot, 'bin'), binDir,
    { required: true, optional: OPTIONAL_BIN_FILES, kindLabel: 'bin' });
  ...
  return { probe, guard, installer, probeBin };
}
...
/**
 * Filter out any hook entry whose JSON serialization contains the marker
 * (so re-running install never leaves the previous version sitting in the
 * array). Then append a fresh entry built by `build`.
 */
function isBudgetGuardEntry(e) {
  const s = JSON.stringify(e);
  return s.includes(HOOK_MARKER) || s.includes(HOOK_MARKER_BASH);
}

function upsertHookArray(arr, build) {
  const filtered = arr.filter((e) => !isBudgetGuardEntry(e));
  filtered.push(build());
  return filtered;
}
...
function buildHookEntry({ binDir, phase, matcher, timeout }) {
  const entry = {
    hooks: [{
      type: 'command',
      command: `${binDir}/guard.mjs ${AGENT} ${phase}`,
      timeout,
    }],
  };
...
function step3_writeMemory({ memoryPath, claudeDir }) {
  ...
  const startEsc = escapeRe(MARK_START);
  const endEsc   = escapeRe(MARK_END);
  const stripRe = new RegExp(`${startEsc}[\\s\\S]*?${endEsc}\\n?`, 'g');
  const stripped = body.replace(stripRe, '').replace(/\n{3,}/g, '\n\n').trimEnd();

  const block = buildProtocolBlock();
  const next = stripped.length === 0 ? block : `${stripped}\n\n${block}`;
  ...
  if (isFile(settingsPath)) {
    ...
    if (cfg && cfg.hooks) {
      ...
      for (const [ev, arr] of Object.entries(cfg.hooks)) {
        ...
        const kept = arr.filter((e) => !isBudgetGuardEntry(e));
