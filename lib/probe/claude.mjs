...
const DEFAULT_USER_AGENT = 'claude-code/2.1.88';
...
export async function fetch(_opts = {}) {
  ...
  return requestJson(`https://${USAGE_HOST}${USAGE_PATH}`, {
    ...
    headers: {
      ...
      'anthropic-beta': 'oauth-2025-04-20',
      'Accept': 'application/json',
      'User-Agent': process.env.BUDGET_CLAUDE_UA || DEFAULT_USER_AGENT,
      'Content-Type': 'application/json',
    },
  });
}
...
export function parse(raw, now, helpers) {
  ...
  return {
    ...
    // warn_bucket_id names the warn_util winner (the bucket actually at the top,
    // across ALL util-bearing buckets incl. non-resettable). It equals bucket_id
    // whenever the top bucket is resettable (the common case), but diverges when
    // a non-resettable bucket is the real max — display/gating should name THIS
    // one so the labeled window matches the warn_util percentage. bucket_id stays
    // the resettable winner for reset-pairing / fingerprint / watchdog.
