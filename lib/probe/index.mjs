...
// Pure ESM, zero external deps, Node >=18. Imported by hook, MCP server, watchdog.
//
...
//   checkThresholds(env)             → {warnOnce, warnRepeat, checkpointLead, hard, errors, source}
//   isoToEpoch(iso) / normalizeUtil(v)
//   readBurnState(agent)                   → parsed burn_<agent>.json state
//
// fetchUsage output is probe_schema 2: top-level `probe_schema: 2` plus
// per-bucket burn fields (burn_rate_pct_per_hour / burn_confident /
// runway_seconds / depleted_at_epoch) appended once enough samples exist.
// Purely additive over the v1 shape — see lib/probe/burn-rate.mjs.
//
// Invariants:
...
import { enrichBuckets, parseBurnState, recordSamples } from './burn-rate.mjs';

const PROVIDERS = { claude, codex };
// probe output schema version. v1 = pre-burn-rate output (implicit, no field);
// v2 adds top-level probe_schema + per-bucket burn_rate_pct_per_hour /
// burn_confident / runway_seconds / depleted_at_epoch (purely additive).
const PROBE_SCHEMA = 2;
...
const burnFile  = (agent) => join(getCacheDir(), `burn_${agent}.json`);
...
export function checkThresholds(env = process.env) {
  ...
  const hard = num(env.BUDGET_HARD, 99, 'hard');
  const defaultCheckpointLead = Math.min(Math.max(95, warnRepeat), hard - 1);
  const checkpointLead = num(env.BUDGET_CHECKPOINT_LEAD, defaultCheckpointLead, 'checkpointLead');
  ...
  check(warnRepeat <= checkpointLead, `warnRepeat(${warnRepeat}) must be <= checkpointLead(${checkpointLead})`);
  check(checkpointLead < hard, `checkpointLead(${checkpointLead}) must be < hard(${hard})`);
  check(hard      <= 100, `hard(${hard}) must be <= 100`);

  if (errors.length) {
    // fall back to defaults so callers can keep running, but surface the errors
    return { warnOnce: 80, warnRepeat: 90, checkpointLead: 95, hard: 99, errors, sources, ok: false };
  }
  return { warnOnce, warnRepeat, checkpointLead, hard, errors, sources, ok: true };
}
...
function emptyResult(agent, now, extra = {}) {
  return {
    ok: false, agent, util: 0, warn_util: 0, bucket_id: '', warn_bucket_id: '',
...
// ─── burn-rate state (IO shell; pure logic lives in burn-rate.mjs) ───────

export function readBurnState(agent) {
  return parseBurnState(readJSON(burnFile(agent)));
}

// Sampling moment = every probe that returns FRESH real data (live fetch, or
// a cache entry written by an earlier live fetch — the unchanged fetched_at
// makes repeated cache hits collapse to no-ops via the non-monotonic rule).
// Fixture probes are fully stateless: they neither sample nor enrich, so test
// and dry-run invocations can never pollute the real burn state.
// Stale results (429 gate / live-fetch failure serving an old cache) are NOT
// sampled but ARE enriched: the rate comes from persisted history while the
// runway is computed over the stale util — consumers can tell from stale=true
// that the burn fields are reference values, not fresh readings.
// Fail-open: any burn-step error returns the result without burn fields.
function applyBurn(burnPath, result, now) {
  try {
    if (result.source === 'fixture') return result;
    if (!Array.isArray(result.buckets) || result.buckets.length === 0) return result;
    let state = parseBurnState(readJSON(burnPath));
    if (result.ok && !result.stale && Number.isFinite(result.fetched_at)) {
      state = withLock(burnPath, () => {
        const lockedState = parseBurnState(readJSON(burnPath));
        const folded = recordSamples(lockedState, result.buckets, result.fetched_at);
        if (!folded.changed) return lockedState;
        try { atomicWriteJSON(burnPath, folded.state); } catch (_) { /* fail-open */ }
        return folded.state;
      });
    }
    return { ...result, buckets: enrichBuckets(state, result.buckets, now) };
  } catch (_) {
    return result;
  }
}
...
export async function fetchUsage(agent, opts = {}) {
  ...
  // Resolve the state-file path BEFORE any await: env (BUDGET_STATE_DIR) is
  // read synchronously at call time, never after a yield point where a
  // test-scoped env override may already have been restored.
  const burnPath = burnFile(agent);
  const result = await fetchUsageCore(agent, opts, now);
  return applyBurn(burnPath, { probe_schema: PROBE_SCHEMA, ...result }, now);
}

async function fetchUsageCore(agent, opts, now) {
  const provider = PROVIDERS[agent];
...
export async function doctor(agent) {
  ...
  out.thresholds = th;
  add('thresholds', th.ok, th.ok
    ? `warnOnce=${th.warnOnce} warnRepeat=${th.warnRepeat} checkpointLead=${th.checkpointLead} hard=${th.hard}`
    : th.errors.join('; '));
